...
class Certificate:
    def __init__(self, jsoncert:dict) -> None:
        ...
        if jsoncert.get('v', None):
            ...
            self.vaccination_certificate = [VaccinationCertificateInfo(x) for x in self.jsoncert['v']]
        elif jsoncert.get('r', None):
            self.ceritificate_type = 'recovery'
            self.recovery_certificate = [RecoveryCertificateInfo(x) for x in self.jsoncert['r']]
